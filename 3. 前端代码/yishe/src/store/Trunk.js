...
export default class Trunk extends Component {
    ...
    deleteItem=(i,that)=>{
        ...
        fetch('http://47.98.163.228:8087/delete',{
            ...
            body:JSON.stringify({nage:i,weizhi:'行李箱',userId:this.props.match.params.id})
        })
                    ...
                    this.state.picture.map((item,i)=>{
                        if(i==this.state.num){
                            return(
                                ...
                                <img src={`http://47.98.163.228:8089/${item}`}style={{width:'120px',height:'120px',border:'1px solid red'}}/>
                            ...
                            return(
                                <div style={{display:'inlinbe-block',position:'relative',width:'120px',height:"120px",margin:'2px',float:'left'}}>
                                <img src={`http://47.98.163.228:8089/${item}`}style={{width:'120px',height:'120px'}}/>
                                <span style={{position:"absolute",color:'red',right:'5px',top:'-3px'}} onClick={this.deleteItem.bind(this,i)}>x</span>
                                </div>
                            )}
                    })
